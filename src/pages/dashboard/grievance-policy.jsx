...
const GrievancePolicy = () => {
  ...
  const { data: policyResponse, isLoading: isFetching, error: fetchError, refetch } = useGetPolicyByTypeQuery('grievance');
...
Last updated: ${new Date().toLocaleDateString()}`
  ...
  const handleSave = async () => {
    try {
      ...
      toast.success('Grievance policy updated successfully');
      setActiveTab('preview');
      refetch();
    } catch (error) {
      console.error('Failed to update grievance policy:', error);
      toast.error(error?.data?.message || 'Failed to update grievance policy');
    }
  };
    ...
    if (isFetching) {
      return (
        <div className="flex justify-center items-center h-64">
          <LoadingIcon />
          <p className="ml-3 text-gray-600 dark:text-gray-400">Loading Grievance policy...</p>
        </div>
      );
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Grievance Policy</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage your grievance policy content and settings</p>
        </div>
      ...
      {activeTab === 'edit' && (
        <div className="space-y-6">
          <Card title="Edit Grievance Policy">
